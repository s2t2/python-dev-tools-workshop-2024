...
> FYI: Mac users who are interested in customizing Terminal app color themes and keyboard shortcuts can reference the professor's [Terminal app configuration notes](mac-terminal-config.qmd). :smiley_cat:
...
> NOTE: Mac users may need to first configure the `code` command by following these [VS Code shell command setup instructions](https://github.com/prof-rossetti/intro-to-python/blob/main/notes/devtools/vs-code.qmd#shell-commands)
...
To download videos from YouTube, try using the [`youtube-dl` command-line utility](/notes/clis/youtube-dl.qmd) (see the linked notes document for installation and usage instructions).
